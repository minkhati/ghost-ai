...
Fetch owned and shared projects server-side using the exisiting project data helper and pass both lists to the sidebar.

No client-side fetching for initial load.

### `Use Project Actions`

Create a hook `hooks/` that manages dialog state and project mutations.
...
The project ID and Loveblocks room ID should stay aligned.
...
- delete refreshes pr redirects correctly
